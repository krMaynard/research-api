...
TABLES: dict[str, TableSpec] = {
    ...
    "sor_comparison": TableSpec(
        "DSA Report Catalogue — self-reported VLOP transparency-report figures compared against "
        "independently aggregated Statements of Reasons (SoRs) from the EU DSA Transparency Database, "
        "by service × content category. Discrepancies are flagged red (>20%), amber (5–20%), ok (≤5%), "
        "or no_data when one side has no figures.",
        "FROM sor_comparison f",
        {
            "period":          "f.period",
            "service_name":    "f.service_name",
            "category_code":   "f.category_code",
            "category_label":  "f.category_label",
            "flag_notices":    "f.flag_notices",
            "flag_tf_notices": "f.flag_tf_notices",
            "flag_own_illegal": "f.flag_own_illegal",
            "flag_own_tos":    "f.flag_own_tos",
            "worst_flag":      "f.worst_flag",
        },
        {
            "rep_notices":      "f.rep_notices",
            "rep_tf_notices":   "f.rep_tf_notices",
            "rep_own_illegal":  "f.rep_own_illegal",
            "rep_own_tos":      "f.rep_own_tos",
            "sor_notices":      "f.sor_notices",
            "sor_tf_notices":   "f.sor_tf_notices",
            "sor_own_illegal":  "f.sor_own_illegal",
            "sor_own_tos":      "f.sor_own_tos",
            "delta_notices":    "f.delta_notices",
            "delta_tf_notices": "f.delta_tf_notices",
            "delta_own_illegal": "f.delta_own_illegal",
            "delta_own_tos":    "f.delta_own_tos",
        },
    ),
    "vlop_registry": TableSpec(
        "VLOP/VLOSE platform registry — catalogue of all 25 designated Very Large Online Platforms "
        "and Search Engines, with tier, reporting period, and links to their published transparency reports.",
        "FROM vlop_registry f",
        {
            "service_name": "f.service_name",
            "platform":     "f.platform",
            "tier":         "f.tier",
            ...
            "transparency_page_url": "f.transparency_page_url",
...
def comparison_page() -> FileResponse:
    """Serve the DSA Report Catalogue comparison page."""
    return _serve_page("comparison.html", "DSA Comparison page")
...
_LOCALIZED_PAGES: dict[str, tuple[str, str, dict[str, list[str]]]] = {
    ...
    "comparison": ("comparison.html", "DSA Comparison page", {}),
}
...
_sor_overview_cache: dict[str, Any] | None = None
_sor_overview_cache_lock = threading.Lock()


def _compute_sor_overview() -> dict[str, Any]:
    conn = _connect_ro()
    try:
        row = conn.execute(
            "SELECT period, is_synthetic FROM sor_comparison LIMIT 1"
        ).fetchone()
        period = row[0] if row else None
        is_synthetic = bool(row[1]) if row else False
        flag_counts = {
            r[0]: r[1]
            for r in conn.execute(
                "SELECT worst_flag, COUNT(*) FROM sor_comparison GROUP BY worst_flag"
            ).fetchall()
        }
        by_service: dict[str, dict[str, int]] = {}
        for svc, flag, n in conn.execute(
            "SELECT service_name, worst_flag, COUNT(*) "
            "FROM sor_comparison GROUP BY service_name, worst_flag"
        ).fetchall():
            by_service.setdefault(svc, {"red": 0, "amber": 0, "ok": 0, "no_data": 0})[flag] = n
        services = [
            {"service_name": svc, **counts}
...
@api_router.get("/comparison/overview")
def comparison_overview() -> dict[str, Any]:
    """Public summary of the DSA Report Catalogue comparison — flag counts by service and overall.
    Memoised (DB is read-only and seeded once at startup)."""
    global _sor_overview_cache
    if _sor_overview_cache is None:
        with _sor_overview_cache_lock:
            if _sor_overview_cache is None:
                _sor_overview_cache = _compute_sor_overview()
    return _sor_overview_cache
